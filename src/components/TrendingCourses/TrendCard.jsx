...
const TrendCard = ({ course }) => {
    return (
        <div className="bg-white rounded-lg border shadow-sm hover:shadow-md overflow-hidden space-y-3 group transition duration-300">
            {/* Image */}
            <img
                src={course.image}
                alt={course.title}
            ...
            <div className="p-4 space-y-3">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    <p className="bg-[#E6F0F9] text-xs font-semibold px-2 py-1 rounded-md text-[#066AC9]">
                        {course.category}
                    </p>
                    <p className="bg-[#24292D] text-white text-xs font-semibold px-2 py-1 rounded-md">
                        {course.level}
                    </p>
                    <CiBookmark className="ml-auto text-xl sm:text-2xl text-gray-600 hover:text-[#066AC9] cursor-pointer" />
                </div>

                {/* Title */}
                <h2 className="text-lg sm:text-xl text-[#24292D] hover:text-[#066AC9] font-bold line-clamp-2 cursor-pointer">
                    {course.title}
                </h2>
                ...
                <div className="flex justify-between text-sm sm:text-base">
                    ...
                    <span className="ml-2">
                        ...
                        <span className="text-xs text-[#8C8B8B]"> (Students)</span>
                    </span>
                </div>
                ...
                <div className="flex flex-wrap gap-4 text-sm sm:text-base">
                    <div className="flex gap-1 items-center">
                        <IoMdTime className="text-lg text-[#D83246]" />
                        <span className="text-[#24292D]">{course.duration}</span>
                    </div>
                    <div className="flex gap-1 items-center">
                        <FiBook className="text-lg text-[#FD7E14]" />
                ...
                <div className="flex items-center justify-between border-t pt-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-sm sm:text-base">{course.author}</span>
                    </div>

                    {/* Price / Cart Button */}
                    <div>
                        <span className="hidden group-hover:block">
                            <button className="px-3 py-2 text-[#0CBC87] hover:text-white font-semibold text-xs sm:text-sm rounded bg-[#E6F8F3] hover:bg-[#0CBC87] duration-500 flex items-center gap-2">
                                <IoCart className="text-lg" /> Add to Cart
                            </button>
                        </span>
                        <span className="font-bold text-xl sm:text-2xl text-[#0CBC87] group-hover:hidden">
...
//                 <h2 className="text-xl text-[#24292D] hover:text-[#066AC9] hover:cursor-pointer font-bold">{course.title.length > 50 ? `${course.title.slice(0, 50)}...` : course.title}</h2>
